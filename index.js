	...
	methods: {
		...
		*/
		...
		"calcStat": function(base, level, iv, ev) {
			...
			var nature = 1;
			var newHp = (Math.floor( ( ( 2 * base['hp'] + iv[0] + Math.floor(ev/4) ) * level )/100 ) + level + 10)
			var newAtk = Math.floor( ( ( Math.floor( ( 2 * base['atk'] + iv[1] + Math.floor(ev/4) ) * level )/100 ) + 5 ) * nature )
			var newDef = Math.floor( ( ( Math.floor( ( 2 * base['def'] + iv[2] + Math.floor(ev/4) ) * level )/100 ) + 5 ) * nature )
			var newSpAtk = Math.floor( ( ( Math.floor( ( 2 * base['spatk'] + iv[3] + Math.floor(ev/4) ) * level )/100 ) + 5 ) * nature )
			var newSpDef = Math.floor( ( ( Math.floor( ( 2 * base['spdef'] + iv[4] + Math.floor(ev/4) ) * level )/100 ) + 5 ) * nature )
			var newSpe = Math.floor( ( ( Math.floor( ( 2 * base['spe'] + iv[5] + Math.floor(ev/4) ) * level )/100 ) + 5 ) * nature )
...
var pokedex = new Vue({
	...
	methods: {
		...
		"addData": function(something, level) {
			...
			var base2 = pokeTable.methods.calcStat(something, this.desiredlevel, this.iv, 0);
			var base = pokeTable.methods.calcStat(something, this.desiredlevel, [0, 0, 0, 0, 0, 0], 0);
			...
			if (base[0] != base2[0] | base[1] != base2[1] | base[2] != base2[2] | base[3] != base2[3] | base[4] != base2[4] | base[5] != base2[5]) {
				...
				myRadarChart.data.labels[0] = "Hitpoint " + base2['0'].toString() + " / " + base['0'].toString();
				document.getElementById("aaa").value = base2['0'];
			}
			myRadarChart.update();
		}
